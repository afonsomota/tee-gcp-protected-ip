...
- `launcher/` — Rust. The open, audited TCB that runs inside the enclave: axum server with `/echo`, `/attestation` (Confidential Space token via `/run/container_launcher/teeserver.sock`), and the HPKE channel (`/hpke-key`, `/hpke/echo`). Keys are generated at boot and bound into the attestation token via `eat_nonce` (`hpke:`/`tls:` prefixed hashes).
