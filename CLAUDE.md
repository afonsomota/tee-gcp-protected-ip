...
- **No committed binary blobs.** The `harness.wasm`/`.sig` test fixture is build output (gitignored), not checked in — rebuilt + signed from `harness/` source by `scripts/build-harness.sh` (CI does this before `cargo test`; locally run it once). An opaque, review-invisible binary in the audited TCB's test tree is exactly the XZ-Utils attack surface; keep build artifacts out of the repo.
